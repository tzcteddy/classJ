...
var calendarRender = function () {
    ...
    $calendarPlan.add(function (today, data) {
        ...
        $calendarItem.html(str).css('width', data.length * 110);
    ...
    $calendarPlan.add(function (today, data, columnId) {
        $btn.on('click', function () {
            var curL = parseFloat($calendarItem.css('left')); //->JQ通过CSS获取的样式值带单位
            ...
            $calendarItem.stop().animate({
                ...
                var strIn = Math.abs(curL / 110),
                    ...
                    $strItem = $link.eq(strIn),
                    $endItem = $link.eq(endIn);
                $strItem.find('a').addClass('bg').parent().siblings().find('a').removeClass('bg');
...
var matchRender = function () {
    return {
        init: function init(columnId, startTime, endTime) {
            $.ajax({
